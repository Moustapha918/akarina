      ...
      {project && (
        <View style={styles.progressSection}>
          ...
          <Text style={styles.progressLabel}>
            {progress}% · {formatMRU(project.collectedAmount, true)} / {formatMRU(project.targetAmount, true)}
          </Text>
        </View>
      )}
      ...
      {investment.status === 'SUCCESS' && project && (
        ...
        >
          {downloadingPdf
            ...
            : <Text style={styles.pdfBtnText}>{t('dashboard.downloadPdf')}</Text>
          }
        </TouchableOpacity>
      )}
    </TouchableOpacity>
...
function DashboardContent() {
  ...
  const KYC_LABELS: Record<KycStatus, string> = {
    NONE: t('dashboard.kycStatus.none'),
    PENDING: t('dashboard.kycStatus.pending'),
    VERIFIED: t('dashboard.kycStatus.verified'),
    REJECTED: t('dashboard.kycStatus.rejected'),
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.section}>
        <Text style={styles.sectionTitle}>{t('dashboard.myInvestments')}</Text>
...
export default function DashboardScreen() {
  ...
  return (
    <GuestGuard
      icon="📊"
      title={t('dashboard.guestTitle')}
      description={t('dashboard.guestDescription')}
    >
      <DashboardContent />
    </GuestGuard>
  );
...
const styles = StyleSheet.create({
  ...
  statValue: { fontSize: 16, fontWeight: '700', color: COLORS.primary, textAlign: 'center' },
  ...
  cardImagePlaceholder: { justifyContent: 'center', alignItems: 'center', backgroundColor: '#EBF5FB' },
  cardImagePlaceholderText: { fontSize: 28 },
  cardInfo: { flex: 1 },
  cardTitle: { fontSize: 14, fontWeight: '700', color: COLORS.textPrimary, marginBottom: 4, lineHeight: 20 },
  ...
  roiValue: { fontSize: 14, fontWeight: '700', color: COLORS.success },
  roiRate: { fontSize: 14, fontWeight: '700', color: COLORS.secondary },
  ...
  pdfBtnText: { fontSize: 12, fontWeight: '600', color: COLORS.primary },

  progressSection: { gap: 4 },
  progressBar: { height: 5, backgroundColor: COLORS.border, borderRadius: 4, overflow: 'hidden' },
  progressFill: { height: '100%', backgroundColor: COLORS.primaryLight, borderRadius: 4 },
  ...
  emptyButton: { backgroundColor: COLORS.primary, borderRadius: 12, paddingHorizontal: 28, paddingVertical: 13 },
  emptyButtonText: { color: '#fff', fontWeight: '700', fontSize: 15 },

  errorText: { color: COLORS.danger, textAlign: 'center', marginTop: 32, fontSize: 14 },
});
